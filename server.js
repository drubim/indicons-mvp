...
app.post('/login', (req, res) => {
  ...
  res.json({ role: user.role, nome: user.nome, codigo: user.codigo });
});
...
app.post('/cadastro', (req, res) => {
  ...
  const codigoIndicador = Math.random().toString(36).substring(2, 8).toUpperCase();
...
app.post('/indicacao', (req, res) => {
  ...
  console.log(`🟡 Nova indicação para ${indicador.nome}: ${nome}`);
